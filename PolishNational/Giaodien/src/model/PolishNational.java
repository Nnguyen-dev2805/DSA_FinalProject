...
public class PolishNational {
    ...
    public String ResultPosfix(String Posfix){
        ...
        for(i=0; i<Posfix.length(); i++){
            ...
            if(Character.isLetter(Posfix.charAt(i))) check=true;
            if((((Character.isLetter(Posfix.charAt(i))) || Character.isDigit(Posfix.charAt(i))) || (Posfix.charAt(i)=='.' ) ||
                    (Posfix.charAt(i)=='-' && i<Posfix.length()-1 && ((Character.isLetter(Posfix.charAt(i+1))) || Character.isDigit(Posfix.charAt(i+1)))))) {
                StringBuilder num = new StringBuilder();
                while ( i<Posfix.length() && (((Character.isLetter(Posfix.charAt(i))) || Character.isDigit(Posfix.charAt(i))) || (Posfix.charAt(i)=='.' ) ||
                        (Posfix.charAt(i)=='-' && i<Posfix.length()-1 && ((Character.isLetter(Posfix.charAt(i+1))) || Character.isDigit(Posfix.charAt(i+1)))))){
                    String ans;
                    if(Character.isLetter(Posfix.charAt(i))) check=true;
                    if(Posfix.charAt(i)=='-'||Posfix.charAt(i)=='.'){
                        ans=String.valueOf(Posfix.charAt(i));
                    }
                    else  ans = String.valueOf(Posfix.charAt(i) - '0');
                    num.append(ans);
                    i++;
                }
